...
if __name__ == '__main__':
    ...
    C_list = [2 ** x for x in range(-14, 12)]  # [2 ** x for x in range(-12, 12)]
    n_jobs = -1
    rff_gamma_list = [2 ** x for x in range(-14, 12)]  # [2 ** x for x in range(-12, 12)]
    ...
    data_generator = DataGenerator(poly_deg=POLY_DEG, size=DATASET_SIZE, dim=DIM, data_folder=DATA_FOLDER,
                                   data_name=DATASET_NAME, results_folder=RESULTS_FOLDER,
                                   xy_noise_scale=[0.05, 0.05], x_range=[0.95, 1.5],
