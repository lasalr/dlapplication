...
if __name__ == '__main__':
    ...
    print('Starting: Parameter tuning for Linear SVC without RFF on {} dataset'.format(ds_name))
    param_grid = {'C': [2 ** x for x in range(-14, 9)],
    ...
    write_csv(path='./Results/', name='param_tune_linearsvc_' + ds_name, start_time=start_time,
              results=gs_model.cv_results_, sortby_col='rank_test_score')

    # Parameter tuning for Linear SVC with RFF
    print('Starting: Parameter tuning for Linear SVC with RFF on {} dataset'.format(ds_name))

    param_grid_rff = {'svc__C': [2 ** x for x in range(-14, 9)],
                      'svc__dual': [False], 'svc__random_state': [RANDOM_STATE],
                      'rff__gamma': [2 ** x for x in range(-14, 9)], 'rff__random_state': [RANDOM_STATE],
                      'rff__n_components': [x for x in range(5, 1001, 100)]}
